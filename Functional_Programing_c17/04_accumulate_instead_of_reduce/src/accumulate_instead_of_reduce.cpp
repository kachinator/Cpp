...
int main()
{
    ...
    message("int results = accumulate(filteredCollection.begin(), filteredCollection.end(), 0, [](int total, int current){ return total + current; })");
    // accumulate takes 4 parameters, container begin-end, then the initial value to accumulate
    // Lambda takes the accumulator value 'total', plus every 'current' value coming from the container
